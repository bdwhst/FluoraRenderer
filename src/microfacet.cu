...
__device__ float fr_complex(float cos_theta_i, math::complex<float> eta)
{
	...
	complex<float> sin2_theta_t = sin2_theta_i / sqr(eta);
	complex<float> cos_theta_t = sqrt(1 - sin2_theta_t);

	complex<float> r_parl = (eta * cos_theta_i - cos_theta_t) / (eta * cos_theta_i + cos_theta_t);
	complex<float> r_perp = (cos_theta_i - eta * cos_theta_t) / (cos_theta_i + eta * cos_theta_t);
	return (norm(r_parl) + norm(r_perp)) * 0.5f;
}
...
__device__ float TRDistribution::D(const glm::vec3& wm) const
{
	using namespace math;
	float tan2theta = tan2_theta_vec(wm);
	if (is_inf(tan2theta))
	{
		return 0;
	}
	float cos4theta = sqr(cos2_theta_vec(wm));
	if (cos4theta < 1e-16f)
		return 0;
	float e = tan2theta * (sqr(cos_phi_vec(wm) / alpha_x) + sqr(sin_phi_vec(wm) / alpha_y));
	return 1 / (pi * alpha_x * alpha_y * cos4theta * sqr(1 + e));
}

__device__ float TRDistribution::lambda(const glm::vec3& w) const
{
	using namespace math;
	float tan2theta = tan2_theta_vec(w);
	if (is_inf(tan2theta))
	{
		return 0;
	}
	float alpha2 = sqr(cos_phi_vec(w) * alpha_x) + sqr(sin_phi_vec(w) * alpha_y);
